...
def update_stops_file(stationsDictionary):
    ...
    if not os.path.exists(FILE_STOPS):
        print(f"❌ Errore: {FILE_STOPS} non trovato.")
        return False
...
def onceEach24H():
    """
    devo:
     1. unzippare "Milano-gtfs.zip"
     2. modificare il file "stops.txt" con le informazioni dello scraper
     3. zippare di nuovo tutti i file (compreso "stops.txt" modificato) in "Milano-gtfs.zip" 
     4. cancellare tutti i file unzippati che tanto non mi servono più
    """
    # 0. Prepariamo i dati
    print("🚀 Avvio procedura di aggiornamento giornaliera...")
    data = getData(DATA_URL)
    if not data:
        print("❌ Impossibile recuperare i dati dallo scraper. Abortito.")
        return
    ...
    extraction_folder = OTP_DATA_FOLDER / "temp_gtfs_extracted"

    try:
        # 1. Unzip
        unzip_gtfs(GTFS_FILE_PATH, extraction_folder)

        # 2. Modifica (aggiorniamo il path globale FILE_STOPS temporaneamente per puntare ai file estratti)
        # Nota: Nel tuo script originale FILE_STOPS era fisso. 
        # Qui lo puntiamo alla cartella temporanea appena creata.
        global FILE_STOPS
        original_file_stops = FILE_STOPS
        FILE_STOPS = extraction_folder / "stops.txt"
        
        update_stops_file(stations_dict)

        # 3. Zip (sovrascrive il vecchio Milano-gtfs.zip)
        ...
        FILE_STOPS = original_file_stops # Ripristiniamo il path originale
